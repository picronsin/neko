...
当前执行调整：根据产品优先级，先推进 UI 视觉与交互基础层；该批次仅改布局、样式和可访问性交互，不改变媒体协议、信令、连接状态机或端口契约。深层 UI SDK/状态拆分仍须等 M1 媒体与网络基线收敛后进行。
...
- 浏览器必须在初始 `signal/request` 中上报可接收的视频 codec；服务端按配置 codec、H.264、VP8 顺序选择已探测且可生成的惰性采集变体，禁止在收到不匹配的 SDP answer 后无限重连。
...
- 需要执行测试时，Agent 只提供可复制的测试命令和预期结果，由用户在其环境中执行；Agent 不直接运行测试命令。收到用户提供的结果后，再据此分析或继续处理。
- 已标记废弃的协议、配置和功能直接删除，不保留兼容层；同步更新文档、示例和测试，避免技术债务。
