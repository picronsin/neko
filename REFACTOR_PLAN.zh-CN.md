...
| M2：可测试的契约与领域核心 | WebSocket 已统一 `{event,payload}` envelope；`protocol/events.json`、`errors.json`、`websocket.schema.json`、`payloads.schema.json` 已建立；Go/TypeScript 事件、错误码和完整事件 payload 类型由生成器同步；服务端/客户端双向 payload 运行时校验、发送前校验、`system/error`、批量 REST 稳定 `error_code` 和契约测试已接入；信令应用层的 SDP/ICE 已与 Pion 类型解耦；`Room`、`Member`、`Session`、`Permission` transport-neutral 模型已接入房间快照和控制权规则，并新增 `RoomRepository`、`MemberRepository`、`SessionRepository` 领域端口及旧会话管理器适配器；官方 OpenAPI Generator 已生成 REST API、模型和错误响应类型，并通过 `sdk/openapi.ts` 接入现有 `AuthClient`/`RoomClient`；本地 Docker Demo 已通过 REST 登录、控制权、稳定错误码及 Chromium/WebSocket/首帧互操作验收 | M2 核心项无未完成项；跨公网、硬件和多平台矩阵属于 M1 外部环境验收范围 | **M2 已完成，本地真实互操作通过** |
...
因此，当前版本不应将 M1/M3 标记为“全部发布完成”：M1 进入真实环境验收阶段，M2 已完成生成式契约、领域隔离实现和本地真实互操作验收，M3 仍处于架构落地前的准备阶段。
...
本批次没有改变公网端口号或 MUX 配置语义；服务端重连宽限/去抖已完成。M2 已补齐事件/错误码和完整事件 payload 生成、双端双向运行时校验、WebSocket/REST 稳定错误码、官方 OpenAPI REST client 生成与 SDK 适配、SDP/ICE transport-neutral 适配，以及 Room/Member/Session/Permission 纯模型和领域 port/repository 边界；本地 Docker Demo 已完成 REST/实时协议互操作验收，不再引入第二套信令 envelope。
...
2. **M2：可测试的契约与领域核心（已完成）**：事件/错误码和完整 payload 生成、唯一 WebSocket envelope、双端双向运行时校验、事件矩阵生成校验、WebSocket/REST 稳定错误码、OpenAPI 生成 REST client 及 SDK 适配、ControlLease、连接状态机、SDP/ICE transport-neutral 适配及 Room/Member/Session/Permission 纯模型和 repository/port 适配器已落地；本地 Docker Demo 已通过 REST 登录、控制权、错误响应、Chromium 登录、WebSocket envelope、首帧和 1280×720 视频互操作验收。
