...
| M2：可测试的契约与领域核心 | WebSocket 已统一 `{event,payload}` envelope；`protocol/events.json`、`errors.json`、`websocket.schema.json`、`payloads.schema.json` 已建立；Go/TypeScript 事件、错误码和完整事件 payload 类型由生成器同步；服务端/客户端 payload 运行时校验、`system/error`、批量 REST 稳定 `error_code` 和契约测试已接入；信令应用层的 SDP/ICE 已与 Pion 类型解耦；`Room`、`Member`、`Session`、`Permission` transport-neutral 模型已接入房间快照和控制权规则，并新增 `RoomRepository`、`MemberRepository`、`SessionRepository` 领域端口及旧会话管理器适配器 | OpenAPI 生成的 TypeScript client 尚未生成并接入（官方生成器已安装，等待 Java 运行时）；仍需将完整 REST API 接入 SDK、补齐跨事件方向的完整契约矩阵和真实协议互操作验证 | **实时契约和领域 port 边界已落地，M2 进行中** |
...
本批次没有改变公网端口号或 MUX 配置语义；服务端重连宽限/去抖已完成。M2 已补齐事件/错误码和完整事件 payload 生成、双端运行时校验、WebSocket/REST 稳定错误码、SDP/ICE transport-neutral 适配，以及 Room/Member/Session/Permission 纯模型和领域 port/repository 边界；下一批接入官方 OpenAPI 生成 client、补全跨事件方向契约矩阵和真实互操作验证，不再引入第二套信令 envelope。
