...
本批次没有改变公网端口号或 MUX 配置语义；服务端重连宽限/去抖已完成。M2 已补齐事件/错误码和完整事件 payload 生成、双端双向运行时校验、WebSocket/REST 稳定错误码、SDP/ICE transport-neutral 适配，以及 Room/Member/Session/Permission 纯模型和领域 port/repository 边界；下一批接入官方 OpenAPI 生成 client 并完成真实互操作验证，不再引入第二套信令 envelope。
...
2. **M2：可测试的契约与领域核心（持续实施）**：事件/错误码和完整 payload 生成、唯一 WebSocket envelope、双端双向运行时校验、事件矩阵生成校验、WebSocket/REST 稳定错误码、ControlLease、连接状态机、SDP/ICE transport-neutral 适配及 Room/Member/Session/Permission 纯模型和 repository/port 适配器已落地；OpenAPI 生成 client 接入和真实协议互操作验证仍未完成。
