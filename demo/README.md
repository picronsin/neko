...
It uses the explicit `balanced` profile (30 fps, 2500 kbit/s) so the standard
M1 quality pipeline is exercised on every demo start. The encoded dimensions
follow the desktop resolution selected in the room.
The demo uses a Docker named volume mounted at `/home/neko/.local/share/neko`;
this keeps chat history and avatars available after the container is recreated.
